...
/*
 * Cafe Q Registration page — matches the Login page's design system.
 *
 * Email/password registration still uses local session since there's no auth API yet.
 * Google Sign-In uses Firebase Authentication and is fully functional.
 */
...
function Register() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Redirect to home after Google sign-in from register page
      navigate("/", { replace: true });
    } catch (error) {
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 bg-surface-container-lowest border border-outline-variant h-[54px] rounded-[14px] hover:bg-surface-container hover:border-primary transition-colors text-label-md font-medium text-on-surface cursor-pointer ${
                  isGoogleLoading ? "opacity-70 cursor-wait" : ""
                }`}
              >
                {isGoogleLoading ? (
                  <>
                    <svg className="w-5 h-5 animate-spin mx-2" viewBox="0 0 24 24" aria-hidden="true">
